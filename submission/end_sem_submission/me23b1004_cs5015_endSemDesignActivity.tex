...
\begin{figure}[H]
    ...
    \caption{Help page with E1 Quick Reference card (dark blue header + 2×2 action grid) placed above the FAQ search. Experienced users can find common tasks without scrolling. Also visible: FAQ categories below. \textit{Block E.}}
\end{figure}
...
\subsection{F1 --- Skip Navigation Link (WCAG 2.4.1)}

\begin{featurebox}{Skip Navigation --- WCAG 2.1 SC 2.4.1: Bypass Blocks}
A skip navigation link is the \textbf{first focusable element} in the page (before the logo). It is \textbf{visually hidden} by default (\texttt{sr-only}) and becomes visible \emph{only} when the keyboard focus lands on it (Tab key). When clicked or activated, it scrolls the page to \texttt{\#main-content}, bypassing all 3 navigation links.

\textbf{Why this matters:} A sighted mouse user never sees the navbar's nav links as an obstacle. But a keyboard-only user (motor disability, power user, screen-reader user) must Tab through every nav link on every page load before reaching content. With 3 nav links + logo + user name + logout = 6 tabs to skip on every page. The skip link reduces this to 1 Tab.

\textit{Implementation:} \texttt{src/components/Navbar.jsx} --- \texttt{<a href="\#main-content" className="sr-only focus:not-sr-only ...">}
\end{featurebox}

\subsection{F2, F3 --- ARIA Labels + Keyboard-Navigable Logo}

\begin{featurebox}{ARIA Labels + Keyboard Navigation}
\textbf{F2 --- \texttt{aria-label} on \texttt{<nav>}:} The navbar element now has \texttt{aria-label="Primary navigation"}, which allows screen-readers to identify this landmark region and let users jump directly to it using landmark navigation (common screen-reader shortcut).

\textbf{F3 --- Logo keyboard-navigable:} The logo div (which acts as a link to home) now has \texttt{role="link"}, \texttt{tabIndex=\{0\}}, \texttt{aria-label="Go to home page"}, and \texttt{onKeyDown} Enter handler. Previously, keyboard users could not activate the logo at all --- it was only clickable.
\end{featurebox}

\subsection{F4 --- Breadcrumb Navigation (already implemented in mid-sem)}

\begin{featurebox}{Breadcrumb Navigation --- Synthesizability}
The DoctorProfile and Booking pages show a breadcrumb: \texttt{Home › Find Doctors › Dr. [Name]} and \texttt{Home › [Doctor] › Book Appointment}. This satisfies the navigation principle that every page must show the user \textit{where they are} and \textit{where they have been}. The breadcrumb also provides an escape hatch for users who arrived via search but want to go back to browsing.
\end{featurebox}
